...
## Regularised models: unpenalised dummies and target lags (FWL)

For `ForecastRidge`, `ForecastLasso`, and `ForecastElasticNet`, dummies are
always unpenalised and remain in their original units. Generated target lags
are unpenalised by default; set `penalise_ar=True` to shrink them. Only exact
target-lag columns generated by `y_lags` qualify.

FWL handles dummies and exempt target lags alongside penalised regressors. Its
`cv` argument accepts an integer or a scikit-learn-compatible splitter. Each
fold learns its projection and optional scaling from the training rows only,
then scores held-out design rows with the unweighted mean fold MSE in the
original target units. These are not recursive multi-step backtests, so choose
a splitter that matches the forecasting task.

An AR-only FWL design skips CV and sets `alpha_` (Ridge) or `best_alpha`
(Lasso and ElasticNet) to `None`. Ordinary non-FWL fixed-alpha fits are
unchanged. For Ridge, the default is 100 log-spaced values from `1e-4` to
`1e2` in `alpha_scaling` units in both CV paths; explicit `alphas` override it.

For the automatic FWL Lasso and ElasticNet grid, see
[Cross-validation for regularised models](models.md#cross-validation-for-regularised-models).
