...
def test_xgb_recursive_noiseless():
    """XGBoost recovers noiseless forecasts and fitted values."""
    ...
    model = rt.models.XGBoost(n_estimators=80, random_state=42)
...
def test_xgb_recursive_with_ar():
    ...
    model = rt.models.XGBoost(n_estimators=40, random_state=42)
    model.fit(y_train, X=X_train)
    forecasts = model.forecast(steps=len(y_test), X=X_test)
...
def test_xgb_direct_h0():
    ...
    model = rt.models.XGBoost(
        n_estimators=40, random_state=42, forecast_strategy="direct", steps=steps
    )
    model.fit(y_train, X=X_train)
