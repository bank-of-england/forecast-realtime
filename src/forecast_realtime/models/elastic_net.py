...
class ForecastElasticNet(LinearRegression):
    ...
    alphas : np.ndarray | list | None
        Alpha values to try when ``cv`` is set. If None, use
        ``ElasticNetCV``'s default candidates. Must be array-like rather than
        a scalar.
    formula : str | None
