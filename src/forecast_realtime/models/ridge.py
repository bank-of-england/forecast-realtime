...
class ForecastRidge(LinearRegression):
    ...
    alphas : np.ndarray | list | None
        Alpha values to try when ``cv`` is set. Defaults to 100 logarithmically
        spaced values from 1e-4 to 1e2, in ``alpha_scaling`` units.
    alpha_scaling : str
    ...
    def _fit_reg(self, y: np.ndarray, X: np.ndarray):
        ...
        if self.cv is None:
            ...
            alphas = self.alphas if self.alphas is not None else self._default_alphas
            model = RidgeCV(
